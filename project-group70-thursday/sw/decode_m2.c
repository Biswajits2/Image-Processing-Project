	...
	for (color=0; color<3; color++) {
		...
		for (i=0; i<height; i+=8) for (j=0; j<width_temp; j+=8) {	//i*width_temp+j+offset_read is the read address of the top left corner of the current 8x8 block
			...
			for (k=0; k<8; k++) for (m=0; m<8; m++) {
				...
				if(i==16 && j==200 && color == 0) printf("FETCH S - %d - %d - Value %d (%x hex)\n", i+k, j+m, temp_matrix1[k][m], temp_matrix1[k][m]);
			}
			...
			for (k=0; k<8; k++) for (m=0; m<8; m++) {
				...
				if(i==16 && j==200 && color == 0){ 
					printf(" S' * C - %d - %d - Value %d (%x hex)\n", i+k, j+m, temp_matrix2[k][m], temp_matrix2[k][m]);
				}
			}
			...
			for (k=0; k<8; k++) for (m=0; m<8; m++) {
				...
				if(i==16 && j==200 && color == 0) printf(" C^T * (S' * C) - %d - %d - Value %d (%x hex)\n", i+k, j+m, temp_matrix1[k][m], temp_matrix1[k][m]);
				//printf(temp_matrix1[k][m]);			
			}
			...
			for (k=0; k<8; k++) for (m=0; m<8; m++) {	//first piece of data goes in high byte
				sram[ ((i+k)*width_temp + j+m) / 2 + sram_offset_address_write ] = ((temp_matrix1[k][m] & 0xff) << 8) | (temp_matrix1[k][m+1] & 0xff);
				if(i==8 && j==0 && color == 0) printf(" S - %d - %d - Value %d (%x hex)\n", i+k, j+m, temp_matrix1[k][m], temp_matrix1[k][m]);
